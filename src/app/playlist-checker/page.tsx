...
import OnlinePing from "@/components/pings/onlinePing";
import OfflinePing from "@/components/pings/offlinePing";
...
export default function PlaylistManager() {
  ...
  async function handleFile(file: any) {
    if (file) {
      ...
      const channelsList = await fetch(`http://localhost:4000/playlistChecker`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ data: dataText })}).then(res => res.json()).then(data => {return data});
        ...
        <div className="grid grid-cols-4 gap-4 bg-gray-100">
          {channels.map((channel: Channel) => (
            <div className="max-w-lg mx-auto my-10 w-4/5	 bg-gray-50 border border-blue-700 rounded-lg shadow-lg p-5" key={channel.url}>
              ...
              {channel.liveStatus !== 200 ? <OfflinePing /> : <OnlinePing />}
